...
begin
    ...
    -- Create DEVELOPER role and place in hierarchy
    ...
    -- Grant warehouse access to DEVELOPER role
    ...
    -- TEST/UAT/PREPROD clones: ETL role deploys via CI/CD
    let dcm_deploy_role VARCHAR;
    if (:clone_type_upper = 'WIP') then
        dcm_deploy_role := :dev_role;
    else
        dcm_deploy_role := :ENVIRONMENT_NAME || '_' || :DOMAIN_NAME || '_ETL';
    end if;

    begin
        execute immediate 'grant role ' || :dcm_deploy_role || ' to role DEPLOYMENT_ADMIN';
        ...
        -- Grant DB_RWC so the deploy role can CREATE objects in all schemas
        execute immediate 'grant database role ' || :clone_db || '.DB_RWC to role ' || :dcm_deploy_role;
    exception
...
begin
    ...
    -- Validate caller is the DOMAIN SYSADMIN
    let domain_sysadmin VARCHAR := :ENVIRONMENT_NAME || '_' || :DOMAIN_NAME || '_SYSADMIN';
